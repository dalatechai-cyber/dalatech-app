...
// Belt-and-suspenders close any unterminated <script>/<style> on the host
// HTML before injecting widget markup. lib/pipeline.js#repairTruncatedHtml
// already does this, but the cost of a duplicate check is zero and the
// cost of a regression here is the entire demo rendering blank (lead #004
// V3, 2026-05-11).
function ensureClosedScriptsBefore(html, anchorRegex) {
  if (!anchorRegex.test(html)) return null;
  const idx = html.search(anchorRegex);
  const before = html.slice(0, idx);
  const stripped = before.replace(/<!--[\s\S]*?-->/g, "");
  const openScripts = (stripped.match(/<script\b[^>]*>/gi) || []).length;
  const closeScripts = (stripped.match(/<\/script\s*>/gi) || []).length;
  const openStyles = (stripped.match(/<style\b[^>]*>/gi) || []).length;
  const closeStyles = (stripped.match(/<\/style\s*>/gi) || []).length;
  let fix = "";
  if (openScripts > closeScripts) fix += "\n</script>".repeat(openScripts - closeScripts);
  if (openStyles > closeStyles) fix += "\n</style>".repeat(openStyles - closeStyles);
  return { idx, fix };
}
...
function injectChatbot(html, options) {
  ...
  for (const re of [/<\/body\s*>/i, /<\/html\s*>/i]) {
    const found = ensureClosedScriptsBefore(html, re);
    if (found) {
      return html.replace(re, (m) => `${found.fix}${widget}\n${m}`);
    }
  }
  return `${html}\n${widget}`;
}
