...
// Native fetch in Node 18+ does NOT time out by default. If Upstash stalls
// mid-connection (or the REST host is unreachable for any reason) the await
// hangs until Vercel kills the lambda — which is exactly how the Telegram
// webhook ended up frozen between "looking up lead #..." and any further
// log. Wrap every Upstash call in an AbortSignal.timeout so a stalled
// request rejects in seconds and the caller's try/catch can run.
const UPSTASH_TIMEOUT_MS = Number(process.env.UPSTASH_TIMEOUT_MS) || 8000;
...
async function redisCommand(args) {
  ...
  try {
    ...
    wrapped.code = err?.name === "TimeoutError" ? "UPSTASH_TIMEOUT" : "UPSTASH_FETCH_ERROR";
    throw wrapped;
  }
  let body;
