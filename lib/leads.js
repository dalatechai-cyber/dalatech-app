...
const STATUS = {
  ...
  SENT: "sent",
  CHOSEN: "chosen",
  // Queued for the local PM2 watcher (routines/watch.js). Set by the
  // Telegram webhook on #NNN finish or CHANGE #NNN; the watcher polls
  // Upstash every 60 s for this status and runs the full production build
  // on Bilguun's machine with no Vercel time limit.
  READY_TO_FINISH: "ready_to_finish",
  // Watcher is actively building the initial production preview.
  FINISHING: "finishing",
  // Production preview deployed; waiting for Bilguun to APPROVE or CHANGE.
  AWAITING_REVIEW: "awaiting_review",
  // Watcher is actively rebuilding after a CHANGE request.
  CHANGING: "changing",
  // Legacy terminal state from the old single-shot finish flow.
  FINISHED: "finished",
  // Bilguun sent APPROVE; site is ready for domain connection.
  APPROVED: "approved",
  // Domain has been queued for connection (purchased + nameservers set, or
  // attached to Vercel awaiting external DNS). Hourly cron polls these.
  DOMAIN_PENDING: "domain_pending",
  // Domain resolves to Vercel; public site is live on the custom name.
...
// Safety net for the three-variant DEMO chain only (generating / html_ready
// / deploying). The production-build flow (FINISHING / CHANGING) is owned
// by the local PM2 watcher in routines/watch.js, which has no Vercel time
// limit; if we re-included those states here, the hourly Vercel cron would
// race the watcher and re-trigger builds that are already in flight on
// Bilguun's machine.
...
// Every domain in DOMAIN_PENDING. The hourly cron iterates this list and
// runs Node's dns.resolve / a Vercel config check on each one; on
// propagation it flips the lead to DOMAIN_LIVE and notifies Bilguun. After
// 72h still pending it sends a one-shot warning.
async function findDomainPendingLeads({ limit = 25 } = {}) {
  const leads = await listLeads();
  return leads
    .filter(l => l.status === STATUS.DOMAIN_PENDING)
    .filter(l => l.domainConnect && l.domainConnect.domain)
    .sort((a, b) => Number(a.number) - Number(b.number))
    .slice(0, limit);
}
