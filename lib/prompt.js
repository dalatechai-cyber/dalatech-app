...
function buildSystemPrompt() {
  return [
    ...
    "",
    "MOBILE FIRST AT 375PX (NON-NEGOTIABLE — most Mongolian visitors arrive on phones)",
    "- The site MUST be fully functional at a 375px viewport width. Build the mental model at 375px FIRST, then scale up to 768px, 1280px, 1920px.",
    "- Every section works on mobile: the header collapses into a hamburger nav at <768px, the hamburger opens a real overlay menu with smooth-scroll anchors that close after a click.",
    "- The hero headline NEVER overflows or clips at 375px. Use clamp() so display type shrinks gracefully. No fixed pixel font-sizes on headings.",
    "- Cards, pricing tiers, services, and team grids stack to a single column at 375px with comfortable spacing (≥16px gap, ≥24px section padding). Two-column splits collapse cleanly.",
    "- All tap targets are at minimum 44px × 44px (Apple HIG). Buttons, nav links, and form-shaped inputs at <768px have padding that achieves this — never a 28px-tall button on mobile.",
    "- Pricing tables remain readable at 375px (font-size ≥14px, numbers ≥18px). Horizontal scroll is acceptable for genuinely tabular data with overflow-x: auto; never let prices clip.",
    "- The Contact section is finger-friendly: the phone number is a tel: link, the email is plain text (no mailto), and visible touch areas are spaced apart so a thumb cannot mis-tap.",
    "- Side padding at 375px is ≥20px. Use clamp(20px, 5vw, 96px) so nothing kisses the viewport edge.",
    "- Test mentally: scroll the page from top to bottom at 375px. If any section overflows, clips, or feels cramped, redesign it.",
    "",
    "VISUAL VARIETY (NON-NEGOTIABLE — no section should look like a template)",
    ...
    "- The hero immediately communicates the brand identity within the first second: strong brand-committed color carrying the surface, opinionated typography (display weight plus a single colored or italic accent word), and ambient CSS motion (drifting blob, breathing gradient, slow rotation, or typewriter on one word). A plain neutral hero is a failure.",
    "- Cycle through these archetypes (no two adjacent sections share an archetype): (1) full-bleed brand-colored band with off-center heading, (2) asymmetric two-column with oversize numeric or display word vs dense copy, (3) breathable centered editorial spread capped at 65ch, (4) varied-size tile grid where at least one tile spans 2 columns, (5) sidebar-and-stream with a vertical pull quote, (6) horizontal scroll strip for menus or products, (7) drenched accent panel with reversed-contrast text.",
    "- No section should look like it came from a template or was auto-generated. If you catch yourself stamping out identical icon + heading + two-line cards, redesign that section into a different archetype.",
    "",
...
function buildUserPrompt(brief) {
  ...
  if (isProduction) {
    lines.push(
      ...
      "- Every section uses Font Awesome icons (large in headings, small in list rows) to add visual texture. NO inline SVG. Decorative shapes come from CSS gradients, conic-gradient, and layered box-shadows."
    );
  }

  // Bilguun-supplied extras and revision history. Only present on the
  // production / change flow (api/telegram.js). Highest-priority client
  // brief content: Bilguun met the client and gathered REAL details.
  const extras = brief.extras || null;
  const photoUrls = Array.isArray(brief.photoUrls)
    ? brief.photoUrls
    : (extras?.photos || []);
  const extraNotes = (extras?.notes || extras?.raw || "").trim();
  const changeHistory = Array.isArray(brief.changeHistory) ? brief.changeHistory : [];

  if (extraNotes) {
    lines.push(
      "",
      "## REAL CLIENT INFORMATION FROM BILGUUN (HIGHEST PRIORITY)",
      "Bilguun met the client in person and collected the following real details. Treat this as the source of truth: real business copy, real prices, real addresses, real names, real hours, real social handles. If anything here conflicts with the original brief above, the information below wins.",
      "",
      extraNotes,
      "",
      "Translate everything into natural Mongolian Cyrillic copy where appropriate. Do NOT paste raw English fragments verbatim. Weave the details into headlines, body paragraphs, the About story, the FAQ answers, the contact section, the footer, and any pricing or service descriptions."
    );
  }
  ...
  if (photoUrls.length > 0) {
    lines.push(
      "",
      "## REAL PHOTO URLS (override the no-image rule for these specific URLs only)",
      "The client has supplied real photos. Use them as actual <img> tags throughout the page. The earlier 'no images' instruction is overridden ONLY for the exact URLs listed below. Do not invent additional images, do not use placeholder image services, do not use stock photo CDNs that are not in this list.",
      "",
      ...photoUrls.map((u, i) => `${i + 1}. ${u}`),
      "",
      "Rules for these images:",
      "- Each <img> must have a meaningful Mongolian alt attribute describing what is in the photo for THIS business.",
      "- Use loading=\"lazy\" and decoding=\"async\" on every <img> except the hero image (the hero image, if you use one, gets fetchpriority=\"high\" and no lazy loading).",
      "- Set explicit width and height attributes on each <img> to prevent layout shift. Pick reasonable values such as 1200x800 for hero, 800x600 for gallery, 600x400 for cards.",
      "- Style images with object-fit: cover and a tasteful border-radius drawn from the design tokens (--radius-md or --radius-lg).",
      "- Distribute the photos across at least two distinct sections (hero, gallery, about, services). Do not dump them all into a single grid.",
      "- The 'no inline SVG' and 'no other CDN libraries' rules still apply. Only the listed URLs are exempt from the image rule."
    );
  }
  ...
  if (changeHistory.length > 0) {
    lines.push(
      "",
      "## REVISION REQUESTS (apply EVERY one of these in addition to the brief above)",
      "Bilguun reviewed earlier versions and asked for the following changes. The newest request is at the bottom and takes precedence when two requests conflict. Treat this as a blocking checklist: every item must be visibly addressed in the new HTML."
    );
    changeHistory.forEach((entry, i) => {
      const at = entry?.at ? ` (${entry.at})` : "";
      const body = (entry?.request || "").trim();
      if (!body) return;
      lines.push("", `### Revision ${i + 1}${at}`, body);
    });
    lines.push(
      "",
      "After applying the revisions, re-read the entire generated HTML and verify each change is present. If a request was vague (eg. 'make the hero bolder'), interpret it through the impeccable design laws and Emil's motion principles, not literally."
    );
  }
