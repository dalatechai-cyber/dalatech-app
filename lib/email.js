...
// Variant labels for the three-demo email. Kept in this module so the
// email copy can evolve independently from lib/process-lead.js.
const VARIANT_LABELS = {
  1: { name: "Загвар №1", flavor: "Minimal — цэвэр орон зай, тогтуун хэв маяг." },
  2: { name: "Загвар №2", flavor: "Bold — хүчтэй типографи, эрчтэй хөдөлгөөн."  },
  3: { name: "Загвар №3", flavor: "Elegant — чамин зай, дээд зэрэглэлийн мэдрэмж." }
};

function normalizeUrlList({ previewUrls, previewUrl }) {
  if (Array.isArray(previewUrls) && previewUrls.length > 0) {
    return previewUrls
      .map((entry, idx) => {
        if (typeof entry === "string") return { designNumber: idx + 1, url: entry };
        return {
          designNumber: Number(entry?.designNumber) || idx + 1,
  ...
  if (typeof previewUrl === "string" && previewUrl) {
    return [{ designNumber: 1, url: previewUrl }];
  }
  return [];
}

function clientEmailHtml({ businessName, fullName, previewUrl, previewUrls, mode }) {
  const safeBiz = escapeHtml(businessName);
  const safeName = escapeHtml(fullName);
  const isFinal = mode === "final";
  const urls = normalizeUrlList({ previewUrls, previewUrl });

  // Final/single-demo emails keep the single-button layout. Multi-demo
  // emails show three labelled buttons so the client can compare and
  // click "Энэ загварыг сонгох" on the one they prefer.
  const showMulti = !isFinal && urls.length >= 2;

  const eyebrow = isFinal
    ? "Бүрэн хувилбар бэлэн боллоо"
    : (showMulti ? "Гурван демо бэлэн боллоо" : "Демо бэлэн боллоо");

  const heroLine = isFinal
    ? `Танай ${safeBiz} вэбсайт амьдарлаа.`
    : (showMulti
        ? `Танай ${safeBiz}-д зориулсан гурван өөр загвар.`
  ...
  const bodyCopy = isFinal
    ...
    : (showMulti
        ? `Сайн байна уу, ${safeName}. Танай бизнест зориулж гурван өөр хэв маягийн демо бэлдлээ. Гурвыг нь нээж харьцуулаад, өөртөө таалагдсан загварынхаа хуудсан дээрх "Энэ загварыг сонгох" товчоог дараарай. Бид 24 цагийн дотор танд холбогдоно.`
  ...
  const note = isFinal
    ...
    : (showMulti
        ? `Сонголтоо хийсний дараа бид танд утсаар холбогдож, контент, домэйн, имэйл маркетинг, удирдах самбарын талаар нарийвчилсан зөвлөгөө өгнө.`
  ...
  if (showMulti) {
    const rows = urls
      .slice()
      .sort((a, b) => a.designNumber - b.designNumber)
      .map(({ designNumber, url }) => {
        const label = VARIANT_LABELS[designNumber] || { name: `Загвар №${designNumber}`, flavor: "" };
        const safeUrl = escapeHtml(url);
        const safeLabel = escapeHtml(label.name);
        const safeFlavor = escapeHtml(label.flavor);
        return `
              <tr>
                <td style="padding:0 32px 14px 32px;">
                  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;background:${BG};border:1px solid ${BORDER};border-radius:14px;">
                    <tr>
                      <td style="padding:18px 20px 16px 20px;">
                        <p style="margin:0 0 4px;font-size:13px;letter-spacing:0.1em;text-transform:uppercase;color:${ACCENT};font-weight:700;">
                          ${safeLabel}
                        </p>
                        <p style="margin:0 0 14px;font-size:13.5px;color:${MUTED};line-height:1.5;">
                          ${safeFlavor}
                        </p>
                        <a href="${safeUrl}" target="_blank" rel="noopener" style="display:inline-block;padding:11px 22px;border-radius:999px;background:${PRIMARY};color:#FFFFFF;font-weight:600;font-size:14px;text-decoration:none;letter-spacing:-0.005em;">
                          Энэ загварыг үзэх →
                        </a>
                        <p style="margin:10px 0 0;font-size:12px;color:${MUTED};word-break:break-all;">
                          ${safeUrl}
                        </p>
                      </td>
                    </tr>
          ...
          <tr>
            <td style="padding:6px 32px 28px 32px;">
              <p style="margin:0;font-size:13px;line-height:1.6;color:${MUTED};">
                Демо хуудас бүрийн доод хэсэгт "Энэ загварыг сонгох" товч байна. Сонголтоо хийхэд бид Telegram дээр мэдэгдэл хүлээж аваад тантай холбогдоно.
              </p>
            </td>
          </tr>`;
  } else {
    const single = urls[0] || { url: "" };
    const safeUrl = escapeHtml(single.url);
...
async function sendClientEmail({ to, businessName, fullName, previewUrl, previewUrls, mode }) {
  const resend = getResend();
  const isFinal = mode === "final";
  const urls = Array.isArray(previewUrls) ? previewUrls.length : 0;
  const subject = isFinal
    ? `Танай ${businessName} вэбсайт амьдарлаа 🎉`
    : (urls >= 2
        ? `Танай ${businessName}-д ${urls} өөр загварын демо бэлэн боллоо 🎉`
        : `Танай ${businessName} вэбсайтын демо бэлэн боллоо 🎉`);
  console.log(`[email] sending client email to=${to} mode=${mode || "demo"} urls=${urls || 1}`);
  return resend.emails.send({
    from: fromAddress(),
    to,
    subject,
    html: clientEmailHtml({ businessName, fullName, previewUrl, previewUrls, mode })
  });
}
