...
async function runPipeline({ brief, previewUrlPlaceholder, leadId, skipReview, skipChatbot }) {
  const placeholder = previewUrlPlaceholder || "https://dalatech-demo.vercel.app";

  let html = await generateHtml(brief);

  // Quality review temporarily disabled for speed/reliability. Re-enable once
  // the pipeline is stable.
  // if (!skipReview) {
  //   try {
  //     const reviewed = await reviewAndFixHtml({ html, brief });
  //     html = reviewed.html;
  //     console.log(`[pipeline] review applied=${reviewed.reviewed}`);
  //   } catch (err) {
  //     console.error("[pipeline] review step failed, continuing with original html:", err?.message || err);
  //   }
  // }

  html = ensureFallbackVisibility(html);

  if (!skipChatbot) {
    const chatEndpoint = process.env.CHAT_ENDPOINT_URL || "https://app.dalatech.online/api/chat";
    html = injectChatbot(html, {
      leadId: leadId || null,
