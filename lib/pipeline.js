...
async function generateHtml(brief, options = {}) {
  const apiKey = process.env.ANTHROPIC_API_KEY;
  if (!apiKey) throw new Error("ANTHROPIC_API_KEY is not set");

  const isProduction = brief.quality === "production";
  const model = options.model || "claude-sonnet-4-6";
  const extraSystem = (options.extraSystem || "").trim();
  const timeoutMs = options.timeoutMs || (isProduction ? 480000 : 280000);
  const maxTokens = options.maxTokens || (isProduction ? 40000 : 32000);

  const client = new Anthropic({ apiKey });
  console.log(`[pipeline] generateHtml quality=${brief.quality || "demo"} variant=${brief.variantStyle || brief.style} business=${brief.businessName} model=${model}`);
...
function decorateHtml(html, { brief, leadId, designNumber, skipChatbot, skipChooser } = {}) {
  ...
  // The chooser bar belongs ONLY on the three demo variants. The production
  // build (api/telegram.js) passes skipChooser:true so the approved site
  // does not nag the visitor to "choose a design".
  if (!skipChooser) {
