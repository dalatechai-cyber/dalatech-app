...
async function triggerNextStage(stageRaw, leadId) {
  ...
  try {
    ...
    console.log(`[process-lead] triggered next stage=${stageRaw} lead=#${leadId}`);
  } catch (err) {
    if (err?.name === "TimeoutError" || err?.name === "AbortError") {
      console.log(`[process-lead] triggered next stage=${stageRaw} lead=#${leadId} (no-wait)`);
      return;
    }
    console.error(`[process-lead] trigger failed stage=${stageRaw} lead=#${leadId}:`, err?.message || err);
  }
}
