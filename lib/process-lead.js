...
const { sendTelegramReply, buildPreviewReadyText, envState: telegramEnvState } = require("./telegram");
...
// Parse a stage label.
// Accepts:
//   - "generate", "deploy", "send" (legacy demo pipeline, defaults variant=1)
//   - "generate:2", "deploy-3", etc. (demo variants, bounded to VARIANTS.length)
//   - "prod:1", "prod:2", ... (production build iterations queued by the
//     Telegram webhook; iteration is unbounded — 1 is the initial finish,
//     2+ are CHANGE iterations).
function parseStage(stageRaw) {
  if (typeof stageRaw !== "string") return null;
  const stage = stageRaw.trim().toLowerCase();
  if (!stage) return null;
  const match = stage.match(/^(generate|deploy|send|prod)(?:[:\-_](\d+))?$/);
  if (!match) return null;
  const kind = match[1];
  const variant = kind === "send" ? null : (match[2] ? Number(match[2]) : 1);
  if (variant !== null) {
    if (!Number.isFinite(variant) || variant < 1) return null;
    // generate/deploy are bounded to the 3 demo variants. prod iterations
    // are unbounded because Bilguun can request as many CHANGE rounds as
    // he wants.
    if ((kind === "generate" || kind === "deploy") && variant > VARIANTS.length) {
      return null;
    }
  }
  return { kind, variant };
...
// Map a lead record into the brief shape that lib/prompt.js expects for a
// production build. Carries the free-form extras Bilguun typed after
// `#NNN finish`, plus accumulated CHANGE history so each iteration has the
// full feedback trail. Moved here from api/telegram.js so the cron stage
// that does the actual build can reuse it.
...
// fails. Moved from api/telegram.js so the cron-driven prod stage can
// drive the build; the webhook now only queues.
async function runProductionBuild({ lead, iteration }) {
  const brief = briefForProduction(lead);

  console.log(`[process-lead] prod build iter=${iteration} lead=#${lead.id} photos=${brief.photoUrls.length} revisions=${brief.changeHistory.length}`);

  const generated = await generateHtml(brief);

  // Haiku quality review pass. On any failure (timeout, malformed output)
  // we keep the original HTML so the build still ships.
  ...
  try {
    ...
    if (review?.html && /<html/i.test(review.html)) {
      ...
      console.log(`[process-lead] prod build #${lead.id} review applied=${!!review.reviewed} length ${generated.length} -> ${reviewed.length}`);
    }
  } catch (err) {
    console.warn(`[process-lead] prod build #${lead.id} review threw, using original:`, err?.message || err);
  }
...
// Best-effort Telegram notification used by the cron prod stage. The
// original user message_id is stored on the lead during the webhook
// handoff (lastUserMessageId); if Telegram lost it, lib/telegram.js's
// sendTelegramReply now falls back to a plain (non-threaded) message,
// so the notification still lands either way.
async function notifyBilguun(lead, text) {
  const env = telegramEnvState();
  if (!env.hasToken || !env.hasChatId) {
    console.warn(`[process-lead] notifyBilguun skipped: token=${env.hasToken} chatId=${env.hasChatId}`);
    return;
  }
  const replyToMessageId = lead?.lastUserMessageId || null;
  try {
    ...
    console.error(`[process-lead] notifyBilguun failed lead=#${lead?.id}:`, err?.message || err);
  }
}

// STAGE: prod-build iteration N. Generates, reviews, decorates, deploys,
// updates the lead, and tells Bilguun the preview is ready. Iteration 1 is
// the initial `#NNN finish`; iterations 2+ are `CHANGE #NNN` rounds.
//
// Idempotency: the queueing webhook writes productionIteration = N on the
// lead before triggering. If we re-enter with the lead already at the
// same iteration AND a matching productionUrl, we skip the regeneration.
async function processProdBuildStage(lead, iteration) {
  if (!Number.isFinite(iteration) || iteration < 1) {
    const error = `invalid iteration ${iteration}`;
    console.error(`[process-lead] prod lead=#${lead.id}: ${error}`);
    return { ok: false, leadId: lead.id, stage: `prod:${iteration}`, error };
  }

  const isInitial = iteration === 1;

  // Idempotency: if a previous attempt already produced the URL for this
  // iteration, just skip. This catches duplicate QStash deliveries and
  // stuck-lead recoveries from the hourly cron safety net.
  if (Number(lead.productionIteration) === iteration && lead.productionUrl && lead.status === STATUS.AWAITING_REVIEW) {
    console.log(`[process-lead] STAGE=prod:${iteration} skipping lead=#${lead.id} already at this iteration url=${lead.productionUrl}`);
    return { ok: true, leadId: lead.id, stage: `prod:${iteration}`, skipped: true };
  }

  console.log(`[process-lead] STAGE=prod:${iteration} lead=#${lead.id} business="${lead.businessName}"`);

  // Make sure status reflects active work — covers the safety-net case
  // where the lead is in an unexpected state.
  ...
  let result;
  try {
    result = await runProductionBuild({ lead, iteration });
  } catch (err) {
    console.error(`[process-lead] prod:${iteration} failed lead=#${lead.id}:`, err?.message || err);
    // Initial finish goes to FAILED (no previous good URL); CHANGE rolls
    // back to AWAITING_REVIEW so Bilguun keeps the prior preview.
    const errorPatch = isInitial
      ? { status: STATUS.FAILED, lastError: `prod:${iteration}: ${err?.message || String(err)}` }
      : { status: STATUS.AWAITING_REVIEW, lastError: `prod:${iteration}: ${err?.message || String(err)}` };
    await updateLead(lead.id, errorPatch);
    const failureText = isInitial
      ? `❌ #${lead.id} амжилтгүй боллоо: ${err?.message || err}`
      : `❌ #${lead.id} засвар амжилтгүй боллоо: ${err?.message || err}\nӨмнөх хувилбар хэвээрээ үлдсэн: ${lead.productionUrl || "—"}`;
    await notifyBilguun(lead, failureText);
    return { ok: false, leadId: lead.id, stage: `prod:${iteration}`, error: err?.message || String(err) };
  }

  const now = new Date().toISOString();
  const previewUrls = isInitial
    ? [result.previewUrl]
    : [...(Array.isArray(lead.productionUrls) ? lead.productionUrls : []), result.previewUrl];

  // For CHANGE iterations, annotate the in-flight change-history entry
  // with the resulting URL so audits can trace each request to a preview.
  ...
  await notifyBilguun(lead, buildPreviewReadyText({
    leadId: lead.id,
    businessName: lead.businessName,
    previewUrl: result.previewUrl,
    iteration
  ...
  console.log(`[process-lead] STAGE=prod:${iteration} ok lead=#${lead.id} url=${result.previewUrl}`);
  return { ok: true, leadId: lead.id, stage: `prod:${iteration}`, previewUrl: result.previewUrl };
}
...
function resumeStageForLead(lead) {
  ...
  // Production-build chain (Telegram webhook → QStash → cron prod:N).
  // The queueing webhook writes productionIteration = N before triggering,
  // so we resume at that same N. Falls back to 1 for an unexpected
  // FINISHING state with no iteration set.
  if (lead.status === STATUS.FINISHING) {
    const iter = Number(lead.productionIteration) || 1;
    return stageLabel("prod", iter);
  }
  if (lead.status === STATUS.CHANGING) {
    const iter = Number(lead.productionIteration) || 2;
    return stageLabel("prod", iter);
  }
...
async function runStage(stageRaw, leadId) {
  ...
  if (parsed.kind === "prod") {
    if (lead.status === STATUS.APPROVED) {
      console.log(`[process-lead] runStage=prod:${parsed.variant} skipping lead=#${leadId} already approved`);
      return { ok: true, leadId, stage: stageRaw, skipped: true };
    }
    return processProdBuildStage(lead, parsed.variant);
  }
