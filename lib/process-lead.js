...
// Fire-and-forget POST to /api/cron with the next stage. Waits briefly so
...
//
// Vercel occasionally drops a self-invocation despite returning 200 (the
// calling isolate is shutting down as the new request arrives). We mitigate
// by adding a unique query param (cache-bust + routing diversity) and
// retrying once on any non-2xx / network error. If the fetch succeeds we
// still rely on the hourly cron + findStuckForStage safety net for the
// rare cases where Vercel routes the request to a busy isolate.
async function triggerNextStage(stageRaw, leadId) {
  ...
  async function attempt(label, timeoutMs) {
    const url = `${CRON_TRIGGER_URL}?t=${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    ...
    console.log(`[process-lead] triggered next stage=${stageRaw} lead=#${leadId}${label}`);
  }
  ...
  try {
    ...
    if (transient) {
      console.log(`[process-lead] triggered next stage=${stageRaw} lead=#${leadId} (no-wait)`);
      // Trigger fetch reached Vercel but didn't return in time; the receiving
      // function is likely already running. Do not retry — would risk a
      // duplicate invocation in addition to the in-flight one. Idempotency
      // checks in runStage protect against duplicates anyway.
      return;
    }
    console.warn(`[process-lead] trigger first attempt failed stage=${stageRaw} lead=#${leadId}:`, err?.message || err);
  }

  // One retry. Slightly longer timeout so we have a real chance of hearing
  // back. The runStage handler is idempotent (skips if previewUrls[N] is
  // already set), so a duplicate invocation here cannot double-deploy.
  await new Promise(r => setTimeout(r, 600));
  try {
    await attempt(" (retry)", 4000);
  } catch (err) {
    if (err?.name === "TimeoutError" || err?.name === "AbortError") {
      console.log(`[process-lead] triggered next stage=${stageRaw} lead=#${leadId} (retry no-wait)`);
      return;
    }
    console.error(`[process-lead] trigger retry failed stage=${stageRaw} lead=#${leadId}:`, err?.message || err);
